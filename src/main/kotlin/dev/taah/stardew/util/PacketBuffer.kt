...
class PacketBuffer @JvmOverloads constructor(val byteBuf: ByteBuf = Unpooled.directBuffer(1)) : ByteBuf() {
    ...
    fun readVarUInt32(): UInt {
        var i = 0
        var j = 0
        while (this.byteArray.size - this.readerIndex() >= 8)
        {
            val num3: Byte = this.readByte()
            i = i or (num3 and 127).toInt() shl j
            j += 7
            if ((num3 and 128.toByte()) == 0.toByte()){
                return i.toUInt()
            }
        }
        return i.toUInt()
